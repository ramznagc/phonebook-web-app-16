...
def get_database_config():
    ...
    if not db_username or not db_password:
        ...
        db_username = ssm.get_parameter(Name="/phonebook/db/username")["Parameter"]["Value"]
        db_password = ssm.get_parameter(Name="/phonebook/db/password", WithDecryption=True)["Parameter"]["Value"]
...
def find_persons(keyword):
    ...
    try:
        ...
        return [
            ...
            for row in rows
        ] or [{"name": "No Result", "number": "No Result"}]
    finally:
        cursor.close()
        connection.close()
